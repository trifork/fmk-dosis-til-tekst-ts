...
async function validateXml(xml: string) {
    const result = await xmlvalidator.validateXML('<?xml version="1.0" encoding="UTF-8"?>' + xml, 'target/fmk-schemas/schemas/fmk-1.4.4-all-types.xsd');
...
describe('XMLGenerator144 M+M+A+N', () => {
    ...
    it('should handle M+M+A+N dose, testing entire xml structure', async () => {
        ...
        expect(formatXml(xml)).to.equal(`<m15:Dosage xsi:schemaLocation="http://www.dkma.dk/medicinecard/xml.schema/2015/01/01 ../../../2015/01/01/DosageForRequest.xsd" xmlns:m15="http://www.dkma.dk/medicinecard/xml.schema/2015/01/01" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
    <m15:Structures>
