...
//    private static final FastDateFormat longDateFormatter = FastDateFormat.getInstance(LONG_DATE_FORMAT, new Locale("da", "DK"));
	...
	public static  trim(number: string): string {
		if(number.indexOf('.')<0 && number.indexOf(',')<0)
			return number;
		if(number.length == 1 || number.charAt(number.length - 1) > '0')
			return number;
		else 
			return TextHelper.trim(number.substring(0, number.length - 1));
	}
	...
	public static  formatLongDate(date: Date): string {
		return TextHelper.weekdays[date.getDay()] + ". " + TextHelper.months[date.getMonth()] + " " + date.getFullYear();
	}
	...
	public static  makeDayOfWeekAndName( startDateOrDateTime: DateOrDateTimeWrapper,  day: DayWrapper,  initialUpperCase: boolean): string {
		...
		if(initialUpperCase)
			...
			name = dayString;
		return "dims"; //new DayOfWeek(dateOnly.getDay(), name, day);
