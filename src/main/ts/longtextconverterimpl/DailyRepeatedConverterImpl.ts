...
export class DailyRepeatedConverterImpl extends LongTextConverterImpl {
	...
	public canConvert(dosage: DosageWrapper ): boolean {
		...
		if(structure.iterationInterval != 1)
			return false;
		if(structure.startDateOrDateTime.equals(structure.endDateOrDateTime))
			return false; 
	...
	public convert(unitOrUnits: UnitOrUnitsWrapper , structure: StructureWrapper ): string  {
		let s = "";		
		this.appendDosageStart(s, structure.startDateOrDateTime);
		
		if(structure.endDateOrDateTime) {
			s += ", gentages hver dag";
			this.appendDosageEnd(s, structure.endDateOrDateTime);
			s += ":\n";
		}
		else {
